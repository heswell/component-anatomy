...
export const RenderVisualiser = ({ children, ...props }) => {
        ...
        <div className="RenderVisualiser-header">
          ...
          <div className="actions">
            <label className="checkbox-classes">
              class names
              <input type="checkbox" checked={showClasses} onChange={toggleClasses}/>
            </label>
            <label className="checkbox-classes">
              aria role
              <input type="checkbox" checked={showRole}  onChange={toggleRole}/>
            </label>
            <label className="checkbox-classes">
              aria attributes
              <input type="checkbox" checked={showAria}  onChange={toggleAria}/>
            </label>
            <label className="checkbox-classes">
              data attributes
              <input type="checkbox" checked={showData}  onChange={toggleData}/>
            </label>
          </div>
        </div>
      ...
      ? tree
          ...
          .map((node, i) => {
            if (typeof node === 'string') {
              if (node.length > 0) {
                return <div className="string-value" key={key}>{`"${node}"`}</div>;
              } else {
                return undefined;
              }
            } else {
              ...
              return (
                <div className="RenderedElement" key={`${key}-${i}`}>
                  <div className="rendered-attributes">
                    ...
                    {showClasses && classes.map((className, j) => (
                      <span
                        className="class-name"
                        key={`class-${j}`}
                      >{`.${className}`}</span>
                    ))}
                    {showAria && Object.entries(ariaAttributes).map(([name, value], k) => (
                      <Attribute
                        key={`aria-${k}`}
                        name={name.slice(5)}
                        prefix="aria"
                    ...
                    {showData && Object.entries(dataAttributes).map(([name, value], k) => (
                      <Attribute
                        key={`data-${k}`}
                        name={name.slice(5)}
                        prefix="data"
...
async function getElementTree(element) {
  ...
  return new Promise(async (resolve, reject) => {
    const { 'aria-owns': ownedId, 'aria-controls': controlledId } = ariaAttributes;
